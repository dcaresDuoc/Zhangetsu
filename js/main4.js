...
thumbnails.forEach(thumbnail => {
    thumbnail.addEventListener('click', event=>{
        ...
        imageContainer.style.backgroundImage = `url('../img/image-product-4-${event.target.id}.jpg')`
    });
...
modalthumbnails.forEach(modalthumbnail =>{
    modalthumbnail.addEventListener('click', event=>{
        ...
        modalImageContainer.style.backgroundImage = `url('../img/image-product-4-${event.target.id.slice(-1)}.jpg')`
...
function  drawProductInModal(){
    ...
    deleteProduct()
    let priceModal = document.querySelector('.cart-modal__price');
    priceModal.innerHTML = `$65.000 x ${lastValue} ---> Total: <span> $${lastValue*65}.000</span>`
}
...
function changeNextImage(imgContainer){
    ...
    imgContainer.style.backgroundImage = `url('../img/image-product-4-${imgIndex}.jpg')`
}
